...
void simulate_error(char *bits) {
    int len = strlen(bits);
    int pos;
    printf("\nDo you want add transition error\n");
    printf("Enter bit postion from (0 to %d), or -1 for NO error: ", len - 1);
    scanf("%d", &pos);
    if (pos >= 0 && pos < len) {
        bits[pos] = (bits[pos] == '0') ? '1' : '0';
        printf("Bit position %d is flipped.\n", pos);
    } else {
