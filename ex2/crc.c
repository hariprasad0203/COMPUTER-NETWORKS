...
int main() {
    ...
    printf("\n \tCRC METHOD\t \n----SENDER SIDE ----\n");
    printf("Enter a string to conversion: ");
    scanf("%s", input);
    str_to_bits(input, data);
    printf("Source data (binary) : %s\n", data);

    printf("Enter generator polynomial bits or press 0 for default bit: ");
    scanf("%s", gen);
    ...
    for (int i = 0; i < gen_len; i++) {
        if (gen[i] == '1') one_count++;
    }

    // Conditions:
    // 1. Minimum 2 bits (gen_len >= 2)
    // 2. x^0 coefficient is 1 (gen[gen_len - 1] == '1')
    // 3. Contains factor (x + 1) -> parity must be even (one_count % 2 == 0)
    if (gen_len >= 2 && gen[gen_len - 1] == '1' && one_count % 2 == 0) {
